...
- **Une seule personne décide des nouvelles grilles** (réglable dans `src/config.js` via `ADMIN_NAME`) : les autres voient le bouton grisé plutôt que de pouvoir relancer une partie en cours
- **« Qui avance ? »** : un tableau en direct montre la progression de chaque collègue connecté — qui s'approche du bingo, qui l'a déjà fait. Un ✕ permet de retirer un doublon (vieil onglet resté ouvert, même collègue connecté deux fois) sans toucher à sa progression
